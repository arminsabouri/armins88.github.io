				...
				foreach ( $mail_bank_table_data as $row ) {
					switch ( $row->type ) {
						case 'email_configuration':
							...
							$email_configuration_array_data['meta_value'] = maybe_serialize( $email_configuration_array );
						...
						case 'settings':
							...
							$settings_array['meta_value'] = maybe_serialize( $settings_data_array );
						...
						case 'roles_and_capabilities':
							...
							$roles_data_array['meta_value'] = maybe_serialize( $roles_capabilities_data_array );
		...
		switch ( $mail_bank_version_number ) {
			case '':
				if ( $wpdb->query( "SHOW TABLES LIKE '" . $wpdb->prefix . "mail_bank'" ) !== 0 ) {
					...
					$wpdb->query( 'DROP TABLE IF EXISTS ' . $wpdb->prefix . 'mail_bank' );// db call ok; no-cache ok.
					mail_bank_table();
					...
					if ( null !== $mail_bank_data ) {
						...
						$update_mail_bank_data_serialize['meta_value'] = maybe_serialize( $update_mail_bank_data );
					...
					$update_plugin_settings_data_serialize['meta_value'] = maybe_serialize( $update_plugin_data );
			...
			default:
				if ( $wpdb->query( "SHOW TABLES LIKE '" . mail_bank() . "'" ) !== 0 && $wpdb->query( "SHOW TABLES LIKE '" . mail_bank_meta() . "'" ) !== 0 ) {// db call ok; no-cache ok.
					...
					if ( array_key_exists( 'roles_and_capabilities', $get_roles_settings_data_array ) ) {
						...
						$roles_capabilities_array['meta_value'] = maybe_serialize( $get_roles_settings_data_array );
						$obj_dbhelper_install_script_mail_bank->update_command( mail_bank_meta(), $roles_capabilities_array, $where );
					}
				}
				if ( $wpdb->query( "SHOW TABLES LIKE '" . mail_bank_email_logs() . "'" ) === 0 ) {// db call ok; no-cache ok.
