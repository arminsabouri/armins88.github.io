...
const Hexdump = () => {
  const [whiteTextCount, setWhiteTextCount] = useState(0)
  const [blackTextCount, setBlackTextCount] = useState(1)

  useEffect(() => {
    const intervalTime = 100 //ms
    const interval = setInterval(() => {
      if (whiteTextCount > genesisHexDump.length + 25) {
        setBlackTextCount(1)
        setWhiteTextCount(0)
      } else {
        setBlackTextCount(blackTextCount + 1)
        setWhiteTextCount(whiteTextCount + 1)
      }
      // setCount(count + 1)
    }, intervalTime)
    return () => clearInterval(interval)
  }, [whiteTextCount, blackTextCount])

  const rowColor = index => {
    if (index === blackTextCount) {
      return '#1A202C'
    } else if (index === whiteTextCount) {
      return 'white'
    }
    return '#81E6D9'
  }
